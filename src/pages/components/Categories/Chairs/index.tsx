...
const Chairs = (props: any) => {
    const navigate = useNavigate();
    let { id } = useParams();
    const [pageData, setPageData] = React.useState(Array); 
   React.useEffect(() => {        
    const filteredData = products.filter((item: { category: string; }) => item.category === 'Chair');
    ...
    <div className="new-in">Chairs</div>
        ...
        <div className="product-box">
          {pageData.length===0 && <div className="product-details-empty">No Products Available in Chairs</div>}          
